    ...
    class Root < Grape::API
        ...
        helpers do
            ...
            def fetchPersonByName(name)
                db = Sequel.postgres('pizzadb', :host => 'localhost', :port => 5432, :max_connections => 10)
                db[:people].where(name: name).all
            end

            def fetchOrdersByType(type)
                db = Sequel.postgres('pizzadb', :host => 'localhost', :port => 5432, :max_connections => 10)
                db[:orders].where(type: type).all
            end
        end
        ...
        resource :people do
            ...
            desc "Fetch Person by Name"
            params do
                requires :name
            end
            post :fetchbyname do
                fetchPersonByName(params[:name])
            end
        end
        ...
        resource :orders do
            ...
            desc "Fetch orders by meat type"
            params do
                requires :type
            end
            post :fetchordersbytype do
                fetchOrdersByType(params[:type])
            end
        end
